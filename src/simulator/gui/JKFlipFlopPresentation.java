...
public class JKFlipFlopPresentation extends ComponentPresentation{

	@SuppressWarnings("unchecked")
	@Override
	public <T> Object execute(T... data) {
		JKFlipFlop jkff = (JKFlipFlop) component;
		jkff.calculate();
		return this;
	}
	...
	public void draw(Graphics g) {
		...
		g.setColor(c);
		g.drawString("J", x+MARGIN_SIDE/2+4, y+height*1/6+5);
		
		p = component.getPort(2);//reset
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x, y + height*5/6, x + MARGIN_SIDE/2, y + height*5/6);
		g.setColor(c);
		g.drawString("K", x+MARGIN_SIDE/2+4, y+height*5/6+5);
