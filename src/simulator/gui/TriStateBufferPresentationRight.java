...
public class TriStateBufferPresentationRight extends ComponentPresentation{
	...
	public void draw(Graphics g) {
		...
		g.drawLine(x+MARGIN_SIDE/2, y,x+width-MARGIN_SIDE/2, y +(height-MARGIN_BOT)/2);
		g.drawLine(x+MARGIN_SIDE/2, y+height-MARGIN_BOT,
				x+width-MARGIN_SIDE/2, y +(height-MARGIN_BOT)/2);
		g.drawLine(x+MARGIN_SIDE/2, y,x+MARGIN_SIDE/2, y+height-MARGIN_BOT);
		
		Port p = component.getPort(0);//enable
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x + (width)/2, y,
				x + (width)/2, y + (height-MARGIN_BOT)*1/4);
		...
		if(!p.getValue().isHighZ())
			g.drawString(Integer.toHexString(p.getValue().getUIntValue()),
				x + width  -10, y+(height-MARGIN_BOT)/2+15);
		else{
			g.drawString("HIGH_Z",
					x + width -20, y+(height-MARGIN_BOT)/2+25);
		}
