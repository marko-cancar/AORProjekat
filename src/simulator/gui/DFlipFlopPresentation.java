...
public class DFlipFlopPresentation extends ComponentPresentation{

	@SuppressWarnings("unchecked")
	@Override
	public <T> Object execute(T... data) {
		DFlipFlop dff = (DFlipFlop) component;
		dff.calculate();
		return this;
	}
	...
	public void draw(Graphics g) {
		...
		p = component.getPort(1);//d
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x, y + height*1/6, x + MARGIN_SIDE/2, y + height*1/6);
		g.setColor(c);
		g.drawString("D", x+MARGIN_SIDE/2+4, y+height*1/6+5);
		
		p = component.getPort(2);//Q
		g.setColor(ColorSignalConverter.convert(p));
