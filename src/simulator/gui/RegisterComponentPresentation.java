...
public class RegisterComponentPresentation extends ComponentPresentation{
	...
	public <T> Object execute(T... data) {
		RegisterComponent reg = (RegisterComponent) component;
		reg.calculate();
		return this;
	}
	...
	public void draw(Graphics g) {
		...
		g.drawLine(x + width - MARGIN_SIDE/2, y + height*3/4, x + width, y + height*3/4);
		g.setColor(c);
		g.drawString("CLK", x+width-MARGIN_SIDE/2-fm.stringWidth("CLK "), y+height*3/4+5);
		
		p = component.getPort(1);//ld
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x + width - MARGIN_SIDE/2, y + height*1/4, x + width, y + height*1/4);
		g.setColor(c);
		g.drawString("LD", x+width-MARGIN_SIDE/2-fm.stringWidth("LD "), y+height*1/4+5);
		
		p = component.getPort(2);//inc
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x, y + height*1/4, x + MARGIN_SIDE/2, y + height*1/4);
		g.setColor(c);
		g.drawString("INC", x+MARGIN_SIDE/2+2, y+height*1/4+5);
		
		p = component.getPort(3);//dec
		g.setColor(ColorSignalConverter.convert(p));
		g.drawLine(x, y + height*3/4, x + MARGIN_SIDE/2, y + height*3/4);
		g.setColor(c);
		g.drawString("DEC", x+MARGIN_SIDE/2+2, y+height*3/4+5);
		
		p = component.getPort(4);//in
		g.setColor(ColorSignalConverter.convert(p));
